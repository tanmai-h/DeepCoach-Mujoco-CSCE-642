...
class FP(gym.Env):
	...
	def reset(self):
		...
		self.mode = 0
		self.prev_obs = obs
		stacked_obs = np.concatenate((self.prev_obs, obs))
		return obs
	...
	def goToGoal(self):
		...
		x_disp = 0.08*np.cos(opp_goal_ang)
		y_disp = 0.08*np.sin(opp_goal_ang)
		proximity_obj_pos = objectPos.copy()
		
		proximity_obj_pos[0] = min(proximity_obj_pos[0]+x_disp, 1.57)
		proximity_obj_pos[1] += y_disp
		proximity_obj_pos[2] = 0.52
		
		grip_obj_ang = (self.get_ang(proximity_obj_pos[0], proximity_obj_pos[1], gripperPos[0], gripperPos[1])+180.0)*np.pi/180.0
		
		proximity_obj_pos[0] = min(proximity_obj_pos[0]+2*x_disp, 1.57)
		proximity_obj_pos[1] += 2*y_disp
		proximity_obj_pos[2] = 0.52
		...
		#if(opp_goal_ang-np.pi/2 <= grip_obj_ang <= opp_goal_ang+np.pi/2):
		if(proximity_obj_dist > 0.1):
			action = [0, 0, 0, 0]
